...
public class PostControllerTest {
    ...
    @Test
    @DisplayName("Should List All Posts When making GET request to endpoint - /api/posts/")
    public void shouldCreatePost1() throws Exception {
		/*
		 * PostResponse postRequest1 = new PostResponse(1L, "Post Name",
		 * "http://url.site", "Description", "User 1", "Subreddit Name", 0, 0,
		 * "1 day ago", false, false); PostResponse postRequest2 = new PostResponse(2L,
		 * "Post Name 2", "http://url2.site2", "Description2", "User 2",
		 * "Subreddit Name 2", 0, 0, "2 days ago", false, false);
		 * 
		 * Mockito.when(postService.getAllPosts()).thenReturn(asList(postRequest1,
		 * postRequest2));
		 */
        mockMvc.perform(get("/api/posts/"))
                .andExpect(status().is(200))
